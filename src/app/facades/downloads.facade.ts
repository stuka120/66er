...
} from "../model/responses/wordpress-media-response.model";
...
export class DownloadsFacade {
  ...
  getDownloadsByTagName(
    tagName: WordpressDownloadTagEnum
  ): Observable<DownloadModel[]> {
    const mapToDownloadModelCollection = (
      wordpressMediaResponseDtoCollection: WordpressMediaResponseModel[]
    ) => {
      return wordpressMediaResponseDtoCollection.map(this.mapToDownloadModel);
    };

    return this.wordpressService.getMediaCollectionForTag$(tagName).pipe(
      filter(collectionIsNotEmpty),
      map(mapToDownloadModelCollection)
    );
  }

  currentDownloads$: Observable<DownloadModel[]> = this.getDownloadsByTagName(
    WordpressDownloadTagEnum.Aktuelles
  );
